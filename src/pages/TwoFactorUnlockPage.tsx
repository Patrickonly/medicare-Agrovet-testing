...
export default function TwoFactorUnlockPage() {
  ...
  const load = async () => {
    ...
    const { data: reqs } = await supabase
      .from("audit_logs")
      .select("id, created_at, action, details, risk_level")
      .eq("user_id", user.id)
      .eq("action", "2fa_support_requested")
    ...
    const { data: res } = await supabase
      ...
      .in("action", ["2fa_support_acknowledged", "2fa_force_reenroll", "2fa_support_denied"])
      .order("created_at", { ascending: false })
      .limit(5);

    setRequests((reqs || []) as SupportRequest[]);
    setResolutions((res || []) as SupportRequest[]);
    setLoading(false);
  };
