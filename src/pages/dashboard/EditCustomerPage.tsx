...
export default function EditCustomerPage() {
  ...
  useEffect(() => {
    if (id) {
      const customer = localDB.customers.getById(id);
      if (customer) {
        setFormData({
          name: customer.name || "",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      localDB.customers.update(id, formData);
      toast({ title: "Customer Updated", description: `${formData.name}'s profile has been updated.` });
      navigate("/dashboard/customers");
    } catch (error) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="pt-6 border-t border-slate-100 flex justify-end gap-3">
              ...
              <Button
                ...
                disabled={loading}
              >
                {loading ? "Saving..." : <><Save className="mr-2 h-4 w-4" /> Update Customer</>}
              </Button>
            </div>
          </form>
        </CardContent>
