...
import { CreditCard, DollarSign, Receipt, FileText, Plus } from "lucide-react";
...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    setSales(localDB.sales.getByOrganizationId(organizationId) || []);
    setCustomers(localDB.customers.getByOrganizationId(organizationId) || []);
  }, [organizationId]);

  const fetchInvoices = useCallback(() => sales, [sales]);
  const { paginatedData, isLoading, search, setSearch, page, setPage, pageSize, totalPages, total } = useData<any>({ fetchFn: fetchInvoices, searchFields: ["invoice_number"] });

  const totalRevenue = sales.reduce((s, i) => s + (i.status === "Completed" ? i.total_amount || 0 : 0), 0);
  const outstanding = sales.reduce((s, i) => s + (i.status === "pending" ? i.total_amount || 0 : 0), 0);
  const insurancePending = 0; // Agrovet doesn't typically have insurance pending

  const getPatient = (pid: string) => customers.find(p => p.id === pid);
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedData.map((inv) => {
                    const status = inv.status === "Completed" ? "paid" : inv.status === "pending" ? "pending" : "overdue";
                    const badge = getBillingStatusBadge(status);
                    const customer = getPatient(inv.customer_id);
                    const total = inv.total_amount || 0;
                    const balance = status === "paid" ? 0 : total;
                    const invoiceNo = inv.invoice_number || inv.ebm_receipt_no || inv.id.split("-")[0].toUpperCase();
                    ...
                    return (
                      <tr key={inv.id} className="border-b border-border last:border-0 hover:bg-secondary/20 transition-colors">
                        ...
                        <Td className="text-muted-foreground">{new Date(inv.timestamp).toLocaleDateString()}</Td>
                        <Td className="font-semibold text-foreground">{total.toLocaleString()} RWF</Td>
                        <Td className="text-medicare-green font-medium">{inv.payment_method}</Td>
